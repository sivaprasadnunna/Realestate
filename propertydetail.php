...
<?php                       //<!-- Enquire Now Button -->
$pid = $_GET['pid']; // Or however you are passing the property ID

// Fetch property information
$propertyQuery = mysqli_query($con, "SELECT * FROM `property` WHERE pid='$pid'");
$propertyRow = mysqli_fetch_array($propertyQuery);

// Get user ID from session
$uid = $_SESSION['uid'];

// Fetch user information
$userQuery = mysqli_query($con, "SELECT * FROM `user` WHERE uid='$uid'");
$userRow = mysqli_fetch_array($userQuery);
?>

<form method="post" action="addlead.php">
    <!-- Hidden fields to pass property and user information -->
    <input type="hidden" name="pid" value="<?php echo $propertyRow['pid']; ?>">
    <input type="hidden" name="uid" value="<?php echo $userRow['uid']; ?>">
    <input type="hidden" name="uname" value="<?php echo $userRow['1']; ?>"> <!-- User's name -->
    <input type="hidden" name="uemail" value="<?php echo $userRow['2']; ?>"> <!-- User's email -->
    <input type="hidden" name="uphone" value="<?php echo $userRow['3']; ?>"> <!-- User's phone -->
    
    <!-- Submit button for Enquire Now -->
    <button type="submit" class="btn btn-success d-none d-xl-block" style="border-radius:30px;">Enquire Now</button>
</form>
